...
| Appwrite clone | 功能认知成熟 | 通用 BaaS 复杂度高，AI/视频/成本治理不是核心 | 不推荐 |
...
- `平台设置`：维护域名、CORS、session policy、OAuth、集成配置、存储和 SMTP 等运行时配置。
...
- 视频翻译：`video-translation/jobs` 和 query/get 接口。
- 视频 retalk：`video-retalk/jobs` 和 query/get 接口。
...
- 存储 provider：平台设置中维护对象存储供应商、默认 provider 和连接测试。
