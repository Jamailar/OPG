...
const DEFAULT_USAGE_WORKERS = 4;
const DEFAULT_USAGE_QUEUE_SIZE = 1000;
const DEFAULT_USAGE_QUEUE_OVERFLOW: UsageQueueOverflowPolicy = 'sync';
...
export class AiGatewayUsageQueueService {
  ...
  private workerCount = DEFAULT_USAGE_WORKERS;
  private maxQueueSize = DEFAULT_USAGE_QUEUE_SIZE;
  private overflowPolicy = DEFAULT_USAGE_QUEUE_OVERFLOW;
  ...
  private async refreshTuning(): Promise<void> {
    try {
      ...
      this.workerCount = this.numberValue(tuning.usage_workers, DEFAULT_USAGE_WORKERS, 1, 32);
      this.maxQueueSize = this.numberValue(tuning.usage_queue_size, DEFAULT_USAGE_QUEUE_SIZE, 1, 100000);
      const overflow = String(tuning.usage_queue_overflow || '').trim().toLowerCase();
      this.overflowPolicy = overflow === 'drop' ? 'drop' : DEFAULT_USAGE_QUEUE_OVERFLOW;
